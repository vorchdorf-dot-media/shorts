...
const ASSETS = `cache__${timestamp}`;
const OFFLINE = `offline__${timestamp}`;

// `build` is an array of all the files generated by the bundler,
// `files` is an array of everything in the `static` directory
const allFiles = [...build, ...files];
const staticAssets = new Set(allFiles);
...
worker.addEventListener('activate', (event: ExtendableEvent) => {
	event.waitUntil(
		caches
			.keys()
			.then(async (keys) => Promise.all(keys.map((key) => key !== ASSETS && caches.delete(key))))
...
worker.addEventListener('install', (event: ExtendableEvent) => {
	event.waitUntil(
		caches
			.open(ASSETS)
			.then((cache) => cache.addAll(allFiles))
			.then(() => {
				worker.skipWaiting();
			})
	);
...
worker.addEventListener('fetch', (event: FetchEvent) => {
	...
	const fetchAssets = async () => {
		...
		/*
    if (!cachedAsset && url.origin === worker.origin && routes.find(route => route.pattern.test(url.pathname))) {
